...
bool initRM3100()
{
    ...
    Wire.requestFrom((uint8_t)RM3100_I2C_ADDRESS, (uint8_t)1);
    if (Wire.available())
    {
        revid = Wire.read();
...
bool readRM3100(int32_t &x, int32_t &y, int32_t &z)
{
    ...
    Wire.requestFrom((uint8_t)RM3100_I2C_ADDRESS, (uint8_t)9);
    if (Wire.available() >= 9)
    {
...
bool initMCP9808()
{
    ...
    Wire.requestFrom((uint8_t)MCP9808_RMT_I2CADDR_DEFAULT, (uint8_t)2);
    if (Wire.available() >= 2)
    {
...
bool initLoRa()
{
    ...
    if (state == RADIOLIB_ERR_NONE)
    {
        ...
        Serial.print(loraConfig.frequency);
        Serial.println(" MHz");
        Serial.print("  Bandwidth: ");
        Serial.print(loraConfig.bandwidth);
        Serial.println(" kHz");
        Serial.print("  Spreading Factor: ");
        Serial.println(loraConfig.spreadFactor);
